...
struct MemoryEditor {
    ...
    bool OptShowOptions = false; // = true   // display options button/context menu. when disabled, options will be locked unless you provide your own UI for them.
    ...
    inline void GotoAddrAndHighlight(size_t addr_min, size_t addr_max)
    {
        GotoAddr = addr_min;
    ...
    struct Sizes {
        ...
        inline Sizes() { memset(this, 0, sizeof(*this)); }
    };

    inline void CalcSizes(Sizes& s, size_t mem_size, size_t base_display_addr)
    {
    ...
    inline void DrawWindow(const char* title, void* mem_data, size_t mem_size, size_t base_display_addr = 0x0000)
    {
        Sizes s;
    ...
    inline void DrawContents(void* mem_data_void, size_t mem_size, size_t base_display_addr = 0x0000)
    {
        if (Cols < 1)
            Cols = 1;
    ...
    inline void DrawOptionsLine(const Sizes& s, void* mem_data, size_t mem_size, size_t base_display_addr)
    {
        IM_UNUSED(mem_data);
        ImGuiStyle& style = ImGui::GetStyle();
    ...
    inline void DrawPreviewLine(const Sizes& s, void* mem_data_void, size_t mem_size, size_t base_display_addr)
    {
    ...
    inline const char* DataTypeGetDesc(ImGuiDataType data_type) const
    {
    ...
    inline size_t DataTypeGetSize(ImGuiDataType data_type) const
    {
    ...
    inline const char* DataFormatGetDesc(DataFormat data_format) const
    {
    ...
    inline static void* EndianessCopyBigEndian(void* _dst, void* _src, size_t s, int is_little_endian)
    {
        if (is_little_endian) {
    ...
    inline static void* EndianessCopyLittleEndian(void* _dst, void* _src, size_t s, int is_little_endian)
    {
        if (is_little_endian) {
            return memcpy(_dst, _src, s);
        } else {
    ...
    inline void* EndianessCopy(void* dst, void* src, size_t size) const
    {
    ...
    inline const char* FormatBinary(const uint8_t* buf, int width) const
    {
        IM_ASSERT(width <= 64);
        size_t out_n = 0;
    ...
    // [Internal]
    inline void DrawPreviewData(size_t addr, const ImU8* mem_data, size_t mem_size, ImGuiDataType data_type, DataFormat data_format, char* out_buf, size_t out_buf_size) const
    {
        uint8_t buf[8];
