...
_bungeegum_ is a cross-platform C++17 library that offers a minimal and modular approach to implementing GUI widgets. It ships with a collection of widgets that mimic those of Flutter. Events and animations help facilitate synchronous and asynchronous operations that modify the GUI state and update changes on the screen. Additionally, _bungeegum_ provides a more advanced interface that enables developers to augment widgets with lower-level functionalities. 
