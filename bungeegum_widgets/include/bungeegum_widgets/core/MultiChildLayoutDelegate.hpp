...
namespace widgets {
    ...
    namespace detail {
        ...
        template <typename idValue_t, template <typename, typename> typename container_t, typename allocator_t = std::allocator<idValue_t>>
        using getConstraintsForChildFunction = decltype(std::declval<delegate_t<idValue_t>>().getConstraintsForChild(std::declval<const BoxConstraints>()));
