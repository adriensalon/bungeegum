...
namespace widgets {
    ...
    Size BoxConstraints::constrainSizeAndAttemptToPreserveAspectRatio(const Size size) const
    {
        ...
        if (_width <= 0.f) {
            error("BoxConstraints", "Impossible to constrain size and attempt to preserve aspect ratio because of illegal width <= 0.f.");
        }
        if (_height <= 0.f) {
            error("BoxConstraints", "Impossible to constrain size and attempt to preserve aspect ratio because of illegal height <= 0.f.");
        }
