...
namespace widgets {
    ...
    struct Offset {
        ...
        // Offset& operatorIntegerDivision(const float operand);
        Offset operator%(const float operand);
        Offset operator*(const float operand);
...
template <>
inline widgets::Offset zero<widgets::Offset> = { zero<float>, zero<float> };

template <>
inline widgets::Offset infinity<widgets::Offset> = { infinity<float>, infinity<float> };
}