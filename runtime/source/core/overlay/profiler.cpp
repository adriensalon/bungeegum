...
namespace detail {
    ...
    namespace {
        ...
        {
            return 0.001f * static_cast<float>(global().pipelines.lifetime_duration.count());
        }
    ...
    {
        frame.setup(global().pipelines.profiler_frame_chronometer);
        resolve.setup(global().pipelines.profiler_resolve_chronometer);
        interact.setup(global().pipelines.profiler_interact_chronometer);
        draw.setup(global().pipelines.profiler_draw_chronometer);
    }
    ...
    void draw_profiler_overlay()
    {
        ...
        if (ImGui::Begin(("profiler" + tag("window_title")).c_str(), NULL, ImGuiWindowFlags_NoCollapse)) {
            ...
            if (ImGui::BeginTabBar(tag("tab_bar").c_str(), tab_bar_flags)) {
                draw_profiler_tab(global().pipelines.profiler_frame_chronometer, frame, _history);
                draw_profiler_tab_with_regex(global().pipelines.profiler_resolve_chronometer, resolve, _history);
                draw_profiler_tab_with_regex(global().pipelines.profiler_interact_chronometer, interact, _history);
                draw_profiler_tab_with_regex(global().pipelines.profiler_draw_chronometer, draw, _history);
            }
            ImGui::EndTabBar();
            ImGui::PushItemWidth(ImGui::GetContentRegionAvail().x);
