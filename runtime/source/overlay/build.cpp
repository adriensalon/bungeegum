...
#if BUNGEEGUM_USE_HOTSWAP
                draw_defines(get_hotswap_defines(), frame_height_without_padding);
                draw_paths<true>("include dirs", get_hotswap_include_directories(), frame_height_without_padding);
                draw_paths<true>("source dirs", get_hotswap_source_directories(), frame_height_without_padding);
                draw_paths<false>("source files", get_hotswap_force_compiled_source_files(), frame_height_without_padding, { { "Source File", { "c", "cpp", "cxx", "c++", "cc" } } });
                draw_paths<false>("libraries", get_hotswap_libraries(), frame_height_without_padding, { { "Library", { "lib", "dll" } } });
#endif
            }
            ImGui::EndTabBar();
        }
