...
namespace detail {
    ...
    reload_state reloader::auto_update(std::wstreambuf* buffer)
    {
    ...
    void reloader::force_update(std::wstreambuf* buffer)
    {
        update_compilation_defines(_swapper.get(), _defines);
        update_compilation_include_directories(_swapper.get(), _include_directories);
        update_compilation_source_directories(_swapper.get(), _source_directories);
    ...
    std::size_t reloader::get_allocated_blocks_count() const
    {
        return _manager.get()->operator->()->GetNumBlocks();
    }

    std::vector<std::string>& reloader::get_defines()
    {
        return _defines.second;
    }

    std::vector<std::filesystem::path>& reloader::get_force_compiled_source_files()
    {
        return _force_compiled_source_files.second;
    }

    std::vector<std::filesystem::path>& reloader::get_include_directories()
    {
        return _include_directories.second;
    }

    std::vector<std::filesystem::path>& reloader::get_libraries()
    {
        return _libraries.second;
    }

    std::vector<std::filesystem::path>& reloader::get_source_directories()
    {
        return _source_directories.second;
    }

    void reloader::register_global(void* data_ptr)
    {
