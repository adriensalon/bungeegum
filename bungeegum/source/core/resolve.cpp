...
float2 resolve_command::resolve_child(const runtime_widget& child_widget, const float2 min_size, const float2 max_size) const
{
    ...
    detail::untyped_widget_data& _child_untyped_widget = detail::global_widgets_manager.get_widget_data(child_widget);
...
void must_resolve()
{
    ...
    detail::global_widgets_manager.resolvables.emplace_back(_data);
}
...
void must_resolve(const runtime_widget& widget)
{
    ...
    detail::global_widgets_manager.resolvables.emplace_back(_widget_data);
}
}