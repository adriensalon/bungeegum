...
namespace detail {
    ...
    {
        if (detail::global_event_container.tickables.contains(raw_event)) {
            detail::global_event_container.notify_erase(raw_event);
        }
    }
            ...
            for (future_iterator _future_it = data.futures.begin(); _future_it != data.futures.end();) {
                if (_future_it->wait_for(std::chrono::seconds(0)) == std::future_status::ready) {
                    ...
                    if (data.futures.empty() && data.shared_futures.empty()) {
                        global_event_container.notify_erase(data.raw_event);
                    }
                } else
                    _future_it++;
            }
...
event<values_t...>& event<values_t...>::trigger(std::future<future_values>&& future_value)
{
    ...
    if (!detail::global_event_container.tickables.contains(_raw_event)) {
        detail::event_update_data& _update_data = detail::global_event_container.tickables[_raw_event];
