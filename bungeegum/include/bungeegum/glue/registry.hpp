...
namespace detail {
    ...
    struct registry {
        ...
        decltype(auto) create_component(const registry_entity entity, args_t&&... args);
        ...
        template <typename component_t>
        component_t& get_component(const registry_entity entity);
        ...
        /// @tparam component_t is the component type to get.
        template <typename component_t>
        const component_t& get_component(const registry_entity entity) const;
        ...
        /// @tparam ...components_t are the component types to select for iteration.
        template <typename... components_t> // exclude filter with strong_typelist
        void iterate_with_entities(const std::function<void(registry_entity, components_t&...)>& iterate_function);

    private:
        entt::basic_registry<registry_entity> _registry;
    };
